...
interface BeyondWorkDetailPageContentProps {
  ...
  listingByLanguage: Record<Language, PostMeta[]>;
}
...
function resolveListing(
  ...
  const preferred = listingByLanguage[language];
  if (preferred.length > 0) {
    return preferred;
  }

  return listingByLanguage.eng.length > 0
    ? listingByLanguage.eng
    : listingByLanguage.fi;
}

function categoryType(
  categoryId?: string,
  category?: string,
): "run_ride" | "visual" | "cooking" | "achievements" | "other" {
  const normalizedId = categoryId?.toLowerCase() ?? "";
  if (normalizedId === "running" || normalizedId === "cycling")
    return "run_ride";
  ...
  if (normalized.includes("running") || normalized.includes("cycling"))
    return "run_ride";
...
function buildMetaItems(
  ...
  if (post.category || post.categoryId) {
    items.push({
      label: t.beyondWorkDetail.category,
      value: localizedCategory(post.categoryId, post.category, t),
    });
  }
  items.push({
    label: t.beyondWorkDetail.date,
    value: formatDate(post.date, language),
  });
  if (post.location)
    items.push({ label: t.beyondWorkDetail.location, value: post.location });

  if (type === "run_ride") {
    if (post.route)
      items.push({ label: t.beyondWorkDetail.route, value: post.route });
    if (post.distance)
      items.push({ label: t.beyondWorkDetail.distance, value: post.distance });
    if (post.slug !== "cycling-to-hailuoto") {
      if (post.duration)
        items.push({
          label: t.beyondWorkDetail.duration,
          value: post.duration,
        });
      if (post.weather)
        items.push({ label: t.beyondWorkDetail.weather, value: post.weather });
      if (post.difficulty)
        items.push({
          label: t.beyondWorkDetail.difficulty,
          value: post.difficulty,
        });
      if (post.gear.length > 0)
        items.push({
          label: t.beyondWorkDetail.gear,
          value: post.gear.join(", "),
        });
    }
  } else if (type === "visual") {
    if (post.theme)
      items.push({ label: t.beyondWorkDetail.theme, value: post.theme });
    if (post.photoCount)
      items.push({ label: t.beyondWorkDetail.photos, value: post.photoCount });
    if (post.gear.length > 0)
      items.push({
        label: t.beyondWorkDetail.cameraGear,
        value: post.gear.join(", "),
      });
    if (post.weather)
      items.push({ label: t.beyondWorkDetail.weather, value: post.weather });
    ...
    if (post.dishType)
      items.push({ label: t.beyondWorkDetail.dishType, value: post.dishType });
    if (post.cuisine)
      items.push({ label: t.beyondWorkDetail.cuisine, value: post.cuisine });
    if (post.timeSpent)
      items.push({
        label: t.beyondWorkDetail.timeSpent,
        value: post.timeSpent,
      });
    if (post.cookingTime)
      items.push({
        label: t.beyondWorkDetail.cookingTime,
        value: post.cookingTime,
      });
    if (post.difficulty)
      items.push({
        label: t.beyondWorkDetail.difficulty,
    ...
    if (post.whatITried)
      items.push({
        label: t.beyondWorkDetail.whatITried,
        value: post.whatITried,
      });
    if (post.whatILearned)
      items.push({
        label: t.beyondWorkDetail.whatILearned,
        value: post.whatILearned,
      });
    if (post.sharedWith)
      items.push({
        label: t.beyondWorkDetail.sharedWith,
        value: post.sharedWith,
      });
  } else if (type === "achievements") {
    if (post.event)
      items.push({ label: t.beyondWorkDetail.event, value: post.event });
    if (post.duration)
      items.push({ label: t.beyondWorkDetail.duration, value: post.duration });
    if (post.result)
      items.push({ label: t.beyondWorkDetail.result, value: post.result });
    if (post.team)
      items.push({ label: t.beyondWorkDetail.team, value: post.team });
  } else {
    if (post.occasion) {
      items.push({ label: t.beyondWorkDetail.occasion, value: post.occasion });
      if (post.temperature)
        items.push({
          label: t.beyondWorkDetail.temperature,
          value: post.temperature,
        });
      if (post.distanceWalked)
        items.push({
          label: t.beyondWorkDetail.distanceWalked,
          value: post.distanceWalked,
        });
    } else {
      if (post.route)
        items.push({ label: t.beyondWorkDetail.route, value: post.route });
...
export function BeyondWorkDetailPageContent({
  slug,
  postsByLanguage,
  listingByLanguage,
}: BeyondWorkDetailPageContentProps) {
  ...
  const shareLabel =
    language === "fi" ? "Jaa tämä merkintä" : "Share this entry";
  ...
  const localizedCategoryLabel =
    post.category || post.categoryId
      ...
      : t.common.journal;
  const isCyclingToHailuoto = slug === "cycling-to-hailuoto";
      ...
      <article className="space-y-10 sm:space-y-12">
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <header className="space-y-4 border-b border-border pb-6 sm:pb-8">
            ...
            <h1 className="max-w-4xl font-serif text-3xl leading-tight text-text sm:text-6xl">
              {post.title}
            </h1>
          ...
          <section className="space-y-6">
            {post.personalNote ? (
              <div className="max-w-reading space-y-2">
                <p className="font-mono text-xs uppercase tracking-label text-muted">
                  {t.beyondWorkDetail.personalNote}
                </p>
            ...
            {post.ingredients.length > 0 ? (
              <div className="space-y-3">
                <p className="font-mono text-xs uppercase tracking-label text-muted">
                  {t.beyondWorkDetail.ingredientsUsed}
                </p>
                      ...
                      >
                        ...
                        <ul className="flex flex-wrap gap-2">
                          {group.items.map((ingredient) => (
                            <li
                              key={`${group.label}-${ingredient}`}
                              className="rounded-md border border-border px-2.5 py-1 text-sm text-text"
                            >
                              {ingredient}
                            </li>
                          ))}
        ...
        <nav
          className="grid gap-3 border-t border-border pt-6 sm:grid-cols-2 sm:pt-8"
          aria-label={t.beyondWorkDetail.journalNavigation}
        >
          {previousPost ? (
            <Link
              href={`/beyond-work/${previousPost.slug}`}
              className="surface-card block min-h-11 p-4"
            >
              <p className="font-mono text-xs uppercase tracking-label text-muted">
                {t.common.previous}
              </p>
          ...
          {nextPost ? (
            <Link
              href={`/beyond-work/${nextPost.slug}`}
              className="surface-card block min-h-11 p-4 sm:text-right"
            >
              <p className="font-mono text-xs uppercase tracking-label text-muted">
                {t.common.next}
              </p>
