...
function resolvePost(language: Language, postsByLanguage: Record<Language, Post | null>): Post | null {
  return postsByLanguage[language] ?? postsByLanguage.eng ?? postsByLanguage.fi;
}

function resolveListing(language: Language, listingByLanguage: Record<Language, PostMeta[]>): PostMeta[] {
  const preferred = listingByLanguage[language];
  if (preferred.length > 0) {
    return preferred;
  }

  return listingByLanguage.eng.length > 0 ? listingByLanguage.eng : listingByLanguage.fi;
}
...
function buildMetaItems(post: Post, t: ReturnType<typeof getDictionary>, language: Language): MetaItem[] {
  ...
  return (
    <article className="space-y-12">
      ...
      {type === "cooking" && post.notesForNextTime.length > 0 ? (
        <section className="space-y-3">
          <p className="font-mono text-xs uppercase tracking-label text-muted">{t.beyondWorkDetail.notesForNextTime}</p>
          <ul className="grid gap-2 sm:grid-cols-2">
