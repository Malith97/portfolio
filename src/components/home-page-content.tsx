...
function toConciseSummary(text: string): string {
  ...
  if (concise.length <= 132) {
    return concise;
  }

  return `${concise.slice(0, 129).trimEnd()}…`;
}

interface HomePageContentProps {
  caseStudiesByLanguage: Record<Language, PostMeta[]>;
  beyondWorkByLanguage: Record<Language, PostMeta[]>;
}

export function HomePageContent({
  caseStudiesByLanguage,
  beyondWorkByLanguage,
}: HomePageContentProps) {
  const { language } = useLanguage();

  const t = useMemo(() => getDictionary(language), [language]);
  const caseStudies = caseStudiesByLanguage[language] ?? [];
  const beyondWorkPosts = beyondWorkByLanguage[language] ?? [];
                  ...
                  <div className="flex flex-1 flex-col">
                    ...
                    <div className="mt-auto pt-5">
                      <Link
                        ...
                        className="text-sm font-medium text-accent transition-colors hover:text-[#f1cc74]"
                      >
