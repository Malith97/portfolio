...
const staticRoutes: Array<{ path: string; priority: number; pageFile: string }> = [
  ...
  { path: "/work-education", priority: 0.6, pageFile: "src/app/work-education/page.tsx" },
];
...
async function getContentRouteEntries(section: "case-studies" | "beyond-work"): Promise<ContentRouteEntry[]> {
  ...
  const entries = await Promise.all(
    mdxFiles.map(async (fileName) => {
      ...
      const frontmatterSlug = typeof data.slug === "string" && data.slug.trim().length > 0 ? data.slug.trim() : null;
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const beyondWorkItems: MetadataRoute.Sitemap = beyondWorkEntries.map((entry) => ({
    url: toCanonical(entry.path),
    lastModified: entry.lastModified,
    changeFrequency: "monthly",
    priority: 0.6,
